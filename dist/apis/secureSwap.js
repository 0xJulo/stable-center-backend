...
const quoteService_1 = require("../1inch_fusion_order/quoteService");
...
router.get("/quote", async (req, res) => {
    ...
    if (!(0, quoteService_1.isSupportedChain)(srcChain) || !(0, quoteService_1.isSupportedChain)(dstChain)) {
        return res.status(400).json({
            success: false,
    ...
    try {
        ...
        const quote = await (0, quoteService_1.getQuoteOnly)(quoteRequest);
...
router.get("/order-status/:orderHash", async (req, res) => {
    ...
    try {
        const status = await (0, quoteService_1.getOrderStatusOnly)(orderHash);
...
router.get("/supported-chains", async (req, res) => {
    try {
        const supportedChains = (0, quoteService_1.getSupportedChains)();
        res.status(200).json({
            success: true,
 ...
 */
router.get("/chains/:chainId/tokens", async (req, res) => {
    ...
    if (!(0, quoteService_1.isSupportedChain)(chainIdNum)) {
        return res.status(400).json({
            success: false,
    ...
    try {
        const tokens = (0, quoteService_1.getChainTokens)(chainIdNum);
        res.status(200).json({
            success: true,
            data: {
                chainId: chainIdNum,
 ...
 */
router.all("/cross-chain*", async (req, res) => {
    res.status(410).json({
        success: false,
...
router.all("/prepare-order*", async (req, res) => {
    res.status(410).json({
        success: false,
...
router.all("/submit-signed-order*", async (req, res) => {
    res.status(410).json({
        success: false,
...
//# sourceMappingURL=secureSwap.js.map