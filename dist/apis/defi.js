...
const defi_utils_1 = require("../lib/defi-utils");
...
router.get("/portfolio/tokens", async (req, res) => {
    ...
    try {
        const result = await (0, defi_utils_1.getPortfolioTokens)(addresses, chain_id);
...
router.get("/portfolio/chart", async (req, res) => {
    const { addresses, chain_id, timerange = "1week", max_points = "50", } = req.query;
    ...
    try {
        const rawData = await (0, defi_utils_1.getPortfolioChart)(addresses, chain_id, timerange);
        // Process the data for optimal chart visualization
        const processedData = (0, defi_utils_1.processChartData)(rawData, parseInt(max_points));
        console.log("✅ Portfolio chart data retrieved and processed successfully");
        console.log(`📊 Processed ${processedData.summary.totalPoints} points from ${processedData.summary.originalPoints} original points`);
        res.status(200).json({
            success: true,
            data: processedData,
...
router.get("/chains", async (req, res) => {
    ...
    res.status(200).json({
        success: true,
        data: {
            chains: Object.entries(defi_utils_1.CHAIN_IDS).map(([name, id]) => ({
                name,
                id,
